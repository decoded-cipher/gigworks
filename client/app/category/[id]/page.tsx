  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetchBusinessesByCategory(slug);
        console.log("Response:", response);
        
        const validBusinesses =
          response.data?.profiles?.map((business: any) => ({
            title: business.title || business.name || "Untitled",
            slug:
              business.slug ||
              business.title?.toLowerCase().replace(/\s+/g, "-") ||
              "untitled",
            src:
              `${ASSET_BASE_URL}/${business.avatar}` ||
