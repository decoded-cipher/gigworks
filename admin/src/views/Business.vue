    ...
    <div class="container mx-auto px-4 py-4 md:py-6">
      ...
      <div class="mb-6 bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
        <div class="flex flex-col sm:flex-row gap-4">
          ...
          <!-- Days filter -->
          <div class="sm:w-1/5">
            <label for="daysFilter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Expiry
              Within (days)</label>
      ...
      <div v-if="totalPages > 1" class="flex justify-center mt-6">
        <div class="inline-flex rounded-md shadow">
          ...
          <span
            class="px-4 py-2 border-t border-b border-gray-300 bg-white dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-200">
...
export default {
  ...
  methods: {
    async fetchBusinesses() {
      try {
        ...
        // Fixed status parameter logic
        const response = await getBusinesses({
          limit: this.limit,
          page: this.currentPage,
          days: this.daysFilter,
          status: this.statusFilter, // Always send status parameter
  ...
  try {
    ...
    const countResponse = await getBusinesses({
      limit: 1,
      page: 1,
      days: this.daysFilter,
      status: this.statusFilter,
      search: this.searchTerm || undefined,
      count_only: true // Ask API for just the count if supported
    });
    
    let totalCount = 0;
    if (countResponse.data.total) {
      totalCount = countResponse.data.total;
    }
    
    // Check if count is too large (optional safety limit)
    const SAFETY_LIMIT = 50000; // Adjust based on your system capabilities
    if (totalCount > SAFETY_LIMIT) {
      if (!confirm(`There are ${totalCount} records matching your filters. Exporting this many records may take a long time or cause browser performance issues. Continue anyway?`)) {
        this.loading = false;
        return;
      }
    }
    
    // Fetch all businesses matching the current filters
    ...
    while (hasMoreData) {
      ...
      if (!response.data || !response.data.profiles || response.data.profiles.length === 0) {
        ...
        allBusinesses = [...allBusinesses, ...response.data.profiles];
        currentPage++;
        
        // Check if we've reached the end
        if (response.data.profiles.length < MAX_PER_BATCH) {
          hasMoreData = false;
        }
      }
    }
    ...
    const formattedData = allBusinesses.map((business) => {
      return {
    ...
    alert('Failed to export data. Please try again or export a smaller dataset.');
  } finally {
    this.loading = false;
  }
}
