...
router.post("/", async (c) => {
  try {
    ...
    if (data.partnerBank) {
      partnerBank = await createPartnerBank({
        ...data.partnerBank,
    ...
    if (data.identityProof) {
      partnerIdProof = await createPartnerIdProof({
        ...data.identityProof,
        partner_id: partner.id,
      });
    }
...
router.patch("/", verifyToken, async (c) => {
  try {
    ...
    if (data.identityProof) {
      ...
      await createPartnerIdProof({
        ...data.identityProof,
        partner_id: partnerData.partner.id,
      });
    }

    const partner: Partner = await updatePartner({
      ...data.partner,
...
router.get("/", verifyToken, async (c) => {
  try {
    ...
    const result = removeFields(partner, [
      "id",
      "user_id",
      "partner_id",
      "updated_at",
...
router.post("/update-status", async (c) => {
  try {
    ...
    const updatedPartner = await updatePartnerStatus(partnerId, status);
    return c.json({
